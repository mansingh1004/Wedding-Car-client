...
const InquiryList = () => {
              ...
              <div key={item._id} className="bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-[#A6715B]/5 hover:shadow-md transition-all group">
                <div className="flex flex-col lg:flex-row justify-between gap-6">
                  ...
                  <div className="lg:w-72 flex flex-col justify-between border-t lg:border-t-0 lg:border-l border-gray-100 pt-6 lg:pt-0 lg:pl-8">
                    ...
                    {/* --- NEW: Status Dropdown --- */}
                    <div className="mt-4">
                        <label className="text-xs font-bold uppercase text-gray-400 block mb-1 ml-1">Current Status</label>
                        <select
                          value={item.status || "Pending"}
                          onChange={(e) => handleStatusChange(item._id, e.target.value)}
                          className={`w-full px-4 py-3 rounded-xl text-sm font-bold border outline-none cursor-pointer transition-all appearance-none text-center ${getStatusColor(item.status)}`}
                        >
  ...
  <option value="Cancelled">Cancelled</option>
                    ...
                    <button 
                      onClick={() => handleDelete(item._id)}
                      className="mt-4 flex items-center justify-center gap-2 text-red-400 hover:text-red-600 hover:bg-red-50 py-3.5 rounded-2xl transition-all font-bold text-sm uppercase tracking-wider"
                    >
                      <Trash2 size={18}/> Remove Inquiry
                    </button>
                  </div>
